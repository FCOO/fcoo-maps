...
(function ($, L, window/*, document, undefined*/) {
    ...
    *********************************************************************/
    L.Control.MapSyncControl = L.Control.extend({
        getState: function(){
            return this.map.options.mapSync ?
                {
                    enabled   : this.map.options.mapSync.enabled,
                    timeOffset: 'timeOffset_' + (this.map.options.mapSync.timeOffset || 0),
                    zoomOffset: 'zoomOffset_' + this.map.options.mapSync.zoomOffset

                } : {};
        },
        setState: function(options){
            this.map._setMapSyncOptions( options );
        }
    });
    ...
    *********************************************************************/
    nsMap.mapSettingGroup_mapSyncOptions = function(header){
        ...
        if (nsMap.setupData.multiMaps.allowDifferentTime){
            ...
            content.push({
                id      : 'timeOffset',
                _label   : {da:'Tidsforskel ift. hovedkort', en:'Time different comp. to main map'},
        ...
        if (!this.$zoomOffsetMultiMapsContainer){
            ...
            this.zoomOffsetMultiMaps = L.multiMaps(this.$zoomOffsetMultiMapsContainer, {
                ...
                update: function( index, map, $mapContainer ){
                    var _this_map = _this.control.map;
