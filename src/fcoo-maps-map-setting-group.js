...
(function ($, L, window/*, document, undefined*/) {
    ...
    var msgSync      = nsMap.msgSync      = 0,
        msgGraticule = nsMap.msgGraticule = 1,
        msgControls  = nsMap.msgControls  = 2;
...
//HER            'bsMapSyncInfoControl': this.map.options.isMainMap ?
//HER                                        {icon: 'fa-clock', text:{da:'Aktuelle tidspunkt', en:'Current Time'}} :
//HER                                        {icon: ['fa-clock', 'fa-sync'], text:{da:'Aktuelle tidspunkt / Synkronisering', en:'Current Time / Synchronizing'}},
    ...
    *****************************************************************************/
    L.Control.BsSetting = L.Control.BsButton.extend({
        options: {
            position       : "topleft",
            icon           : [nsMap.mapSettingIcon, ns.iconSub('fa-map', 'fa-sync icon-active font-size-0-65 font-weight-bold')],
            bigIcon        : true,
        ...
        onAdd: function(map){
            ...
            //Finds the two stacked icon-divs
            var icons = $(result).children('.container-stacked-icons').addClass('fa-no-margin');
            this.noSyncIcon = $(icons[0]);
            this.syncIcon = $(icons[1]);
        ...
        onChange: function(options){
            var syncWithMainMap = this._map.mapSyncControl && this._map.mapSyncControl.getState().enabled;
            this.noSyncIcon.toggle(!syncWithMainMap);
            this.syncIcon.toggle(!!syncWithMainMap);
        }
